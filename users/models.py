...
class CustomUserManager(BaseUserManager):
    """Кастомный менеджер для модели пользователя CustomUser. Переопределяет стандартные
    методы создания пользователя и суперпользователя для работы с email вместо username."""
    ...
    def create_user(self, email, password=None, **extra_fields):
        ...
        **extra_fields: Дополнительные поля модели пользователя
        Возвращает: CustomUser: Созданный объект пользователя
        Исключения: ValueError: Если email не указан."""
        if not email:
    ...
    def create_superuser(self, email, password=None, **extra_fields):
        ...
        **extra_fields: Дополнительные поля модели пользователя
        Возвращает: CustomUser: Созданный объект суперпользователя
        Исключения: ValueError: Если не установлены флаги is_staff или is_superuser."""
...
class CustomUser(AbstractUser):
    """Кастомная модель пользователя с аутентификацией по email. Наследует от AbstractUser
    и заменяет поле username на email в качестве основного идентификатора пользователя.
    Атрибуты:
    ...
    def __str__(self):
        ...
        Возвращает: str: Email адрес пользователя."""
        return self.email


class EmailConfirmation(models.Model):
    """Модель для хранения токенов подтверждения email адреса. Создается при регистрации нового пользователя
    и удаляется после успешного подтверждения email.
    Атрибуты:
    ...
    def __str__(self):
        ...
        Возвращает: str: Email пользователя и дата создания."""
