...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            user.save()
            self.stdout.write(self.style.SUCCESS("✅ Создан демо-пользователь: demo@example.com/demo123"))
        else:
        ...
        demo_entries = [
            {
                ...
                "content": "Это было прекрасное утро. "
                "Я решил начать вести дневник чтобы сохранять свои мысли и воспоминания.",
        ...
        for entry_data in demo_entries:
            ...
            if not Entry.objects.filter(title=entry_data["title"], author=user).exists():
                entry = Entry.objects.create(title=entry_data["title"], content=entry_data["content"], author=user)
        ...
        self.stdout.write(self.style.SUCCESS(f"🎉 Успешно создано {created_count} демо-записей!"))
        self.stdout.write(self.style.SUCCESS("👤 Для входа используйте: email: demo@example.com, password: demo123"))
